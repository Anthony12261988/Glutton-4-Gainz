...
export const recipeSchema = z.object({
  ...
  description: z
    ...
    .nullable(),
  protein_g: z
    .number({ message: "Protein must be a valid number" })
    .min(0, "Protein cannot be negative")
    .max(500, "Protein value is too high"),
  carbs_g: z
    .number({ message: "Carbs must be a valid number" })
    .min(0, "Carbs cannot be negative")
    .max(500, "Carbs value is too high"),
  fat_g: z
    .number({ message: "Fat must be a valid number" })
    .min(0, "Fat cannot be negative")
    .max(200, "Fat value is too high"),
  calories: z
    .number({ message: "Calories must be a valid number" })
